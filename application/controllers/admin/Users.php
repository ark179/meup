...
class Users extends ADMIN_Controller {
    ...
    public function get_users() {
        ...
        if (count($result) > 0) {
            ...
            fputcsv($fp, array('Email', 'FirstName', 'LastName', 'PrimaryUse', 'Gender','DOB','Installation Date','App Version','Status'));

            foreach ($result as $user) {
               
                fputcsv($fp, array($user['email'], $user['first_name'], $user['last_name'], $user['primary_use'],(($user['user_gender']=='M')?'Male':(($user['user_gender']=='F')?'Female':'Other')), $user['user_birthdate'],$user['created_date'],$user['app_version'],($user['status']==1)?'Active':'Inactive'));
            }
